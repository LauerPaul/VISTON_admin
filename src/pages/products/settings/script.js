/**
* @vuedoc
* @module pages/products/settings
* @see @/pages/products/settings
*
* @version 1.0
* @desc Страница настроек раздела "продукции"
*
...
const data = {
	...
	*/
	urlGetSettings: '/products/settings',
	valid: true,
	loading: true,
	access: {
		edit: false
...
const methods = {
	...
	**/
	changeAll(index){
		this.$log.debug('page \'Products settings\' (@/pages/products/settings) - method init');
	...
	**/
	...
	/**
	* 	@desc <strong style="color:red; font-size: 18px;">ⓘ</strong> Запрос настроек страницы продукции
	*	@method getSettings
	**/
	getSettings (lng = false){
		this.$log.info('page \'Products settings\' (@/pages/products/settings) - method init');
		...
		return this.axios({
            ...
            if(response.data.status == "ERROR") {
				this.$log.error('page \'Products settings\' (@/pages/products/settings) - AJAX error (GET)');
                this.$logger('error', 'Произошла ошибка при загрузке настроек продукции. Ошибка: ' + response.data.error)
            	this.$notify.error('Произошла ошибка при загрузке настроек...')
            }
            else {
				this.$log.debug('page \'Products settings\' (@/pages/products/settings) - AJAX success');
		...
		if(this.access.edit){
			if(this.$refs.form.validate()){
				...
				return this.axios({
	                ...
	                if(response.data.status == "ERROR") {
						this.$log.error('page \'Products settings\' (@/pages/products/settings) - AJAX error');
	                    this.$logger('error', 'Произошла ошибка при сохранении настроек продукции. Ошибка: ' + response.data.error)
...
export default {
	...
	*/
	...
	// Head
	metaInfo: {
		title: 'Админ панель - Настройки раздела "продукция"'
	},
	...
	* @event module:pages/products/settings~Page <strong>Products settings</strong> mounted
	*/
	mounted: function(){
		this.$log.info('page \'Products settings\' (@/pages/products/settings) - mounted hook init');
