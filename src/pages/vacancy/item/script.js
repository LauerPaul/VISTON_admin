/**
* @vuedoc
* @module pages/vacancy/item
* @see @/pages/vacancy/item
*
* @version 1.0
* @desc Страница редактирования вакансии
*
...
const data = {
	...
	*
	*/
	getItemUrl: '/vacancy/item/',
	saveItemUrl: '/vacancy/save/',
	id: 0,
	step: 0,
	loading: true,
	access: {
...
const methods = {
	...
	**/
	getItemData(){
		this.$log.info('page \'Vacatcy item\' (@/pages/vacancy/item) - method init');
		...
		return this.axios({
            ...
            if(response.data.status == "ERROR") {
				this.$log.error('page \'Vacatcy item\' (@/pages/vacancy/item) - AJAX error');
            	this.notify = 'Произошла ошибка при загрузке данных...'
                this.$store.commit('error', response.data.error);
            }
            else {
				this.$log.debug('page \'Vacatcy item\' (@/pages/vacancy/item) - AJAX success');
				this.item = ''
	...
	submit() {
		...
		if(this.$refs.form.validate()){
			...
			return this.axios({
                ...
                if(response.data.status == "ERROR") {
					this.$log.error('page \'Vacatcy item\' (@/pages/vacancy/item) - AJAX error');
					this.$logger('error', 'Произошла ошибка при сохранении данных вакансии. Ошибка: ' + response.data.error)
	            	this.$notify.error('Произошла ошибка при сохранении данных вакансии.')
	            	console.log(response.data.error);
                } else {
					this.$log.debug('page \'Vacatcy item\' (@/pages/vacancy/item) - AJAX success');
                    this.$notify.success('Изменения сохранены!');
                    // this.getItemData();
		            this.$router.push({name: 'vacancyList'});
 				}
            });
...
export default {
	...
	*/
	mounted(){
		this.$log.info('page \'Vacatcy item\' (@/pages/vacancy/item) - mounted hook init');
