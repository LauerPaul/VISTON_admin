/**
* @vuedoc
* @module pages/vacancy/list
* @see @/pages/vacancy/list
*
* @version 1.0
...
const data = {
	...
	*	@property {array} vacancy - Список услуг (Зарезервированная переменная)
	*
	...
	*/
	urlGet: '/vacancy/list/',
	removeItemUrl: '/vacancy/remove/',
	newItemUrl: '/vacancy/new/',
	loading: true,
		...
		if(this.$access('access')){
			return this.axios({
	            ...
	            if(response.data.status == "ERROR") {
					this.$log.error('page \'vacancy list\' (@/pages/vacancy/list) - AJAX error');

	            	this.notify = 'Произошла ошибка при загрузке данных...'
	                this.$store.commit('error', response.data.error);
	            }
	            else {
					this.$log.debug('page \'vacancy list\' (@/pages/vacancy/list) - AJAX success');
	...
	**/
	removeAlert(){
		...
		if(this.$access('remove')){
			...
			this.dialogRemove.text = 'Вы действительно хотите удалить запись вакансии с названием<br><strong>"'+ this.selectItem.title +'"</strong> - <strong>ID '+ this.selectItem.id +'</strong>? <br> <br> Запись будет удалена без возможности восстановления.'
...
export default {
	...
	*/
	...
	// Head
	metaInfo: {
		title: 'Админ панель - Список вакансий - Карьера'
	},

	/**
	* @desc ▶ Hook reporting
	* @event module:pages/vacancy/list~Page <strong>vacancy list</strong> mounted
	*/
	mounted(){
		this.$log.info('page \'vacancy list\' (@/pages/vacancy/list) - mounted hook init');
